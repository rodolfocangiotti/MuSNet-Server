...
void UDPResponse::operator()(RequestInfo& r) {
  ...
  if (reqstDatagram.header() == AUDIO_STREAM_DATA) {
    ...
    if (respTID > 0) {
      ...
      // XXX Do some test by responding with the same TID as the request...
      //writableReqstDatagram.buildAudioStream(t, respTID, toClient);
      writableReqstDatagram.buildAudioStream(t, reqstTID, toClient);
      ...
      for (int i {0}; i < REDUDANCY_FACTOR; i++) { // TODO APPLY REDUDANCY...
